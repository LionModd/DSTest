...
complete.boxers %>%
  ...
  colSums()
```
Ok now we have about 67k rows of data regarding a boxer's age, weight, height and reach with no missing data. Let's see if there are any outliers using `plot_num()` in the `funModeling` package and `describe()` in the `Hmisc` package.
```{r, echo = TRUE}
...
Once filtered out this will remedy the skewed data. We'll also recheck the distributions.
```{r, echo = TRUE}
complete.boxers %>%
